...
from utils import get_last_n_emails, get_value_for_key # Funciones auxiliares
...
with st.container():
    ...
    with col1:
        # Crear un botón para redireccionar a la autentificación OAuth 2.0
        st.link_button("Obtener OAuth2.0", REDIRECT_URL)
        # Crear un campo de entrada de texto para el token
        token_str = st.text_input('Ingrese el token obtenido del OAuth2.0')

        # Verificar si se ingresó un token
        if token_str:

            # Convertir el token de string a JSON
            token_json = json.loads(token_str)

            # Definir los alcances (scopes) para el acceso a Gmail
            SCOPES = ['https://www.googleapis.com/auth/gmail.readonly']

            # Crear credenciales a partir de la información del token
